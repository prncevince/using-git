---
title: "Modern M&A - Version Control - Using Git"
subtitle: "⚔<br/>with xaringan"
author: "The Authors Names ... just meta-data"
date: "2019/08/16 (updated: `r Sys.Date()`)"
output:
  xaringan::moon_reader:
    seal: false
    css: [theme/src/css/kunoichi.css,       theme/src/css/ninjutsu.css,
          theme/src/css/ninpo.css,          theme/src/css/default.css,
          theme/src/css/default-fonts.css,  theme/src/css/custom-fonts.css,
          theme/usr/css/custom.css,         assets/css/deck.css]
    chakra: theme/src/js/remark.min.js
    lib_dir: libs
    nature:
      ratio: 16:10
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
      navigation:
        scroll: false
---

```{r setup, include=FALSE}
options(htmltools.dir.version = FALSE)
knitr::opts_chunk$set(fig.path = "assets/img/")
library(anicon)
```

```{asis notes, include=FALSE}
# NOTES:

```

```{r child="slides/theme.Rmd"}
```

---
```{r child="slides/1-title.Rmd"}
```

---
```{r child="slides/theme-series.Rmd"}
```

---
```{r child="slides/2-preface.Rmd"}
```

---
```{r child="slides/3-setup.Rmd"}
```

---
```{r child="slides/4-git-and-friends.Rmd"}
```

---
```{r child="slides/5-ssh-keys.Rmd"}
```

---
```{r child="slides/6-ssh-config.Rmd"}
```

---
```{r child="slides/7-git-config.Rmd"}
```

---
```{r child="slides/8-getting-started.Rmd"}
```
???
Demo by creating a new repo.
Then, delete it.
---
```{r child="slides/9-rproject-rstudio-git.Rmd"}
```
???
Demo with https://esgovcloud.com/mAndA/clone-me.
---
```{r child="slides/10-three-trees.Rmd"}
```

---
```{r child="slides/11-git-init.Rmd"}
```

---
```{r child="slides/12-git-add.Rmd"}
```

---
```{r child="slides/13-git-commit.Rmd"}
```

---
```{r child="slides/14-git-pull.Rmd"}
```

---
```{r child="slides/15-git-reset.Rmd"}
```

---
```{r child="slides/16-git-revert.Rmd"}
```

---
```{r child="slides/17-git-tag.Rmd"}
```

---

class: center middle

# .x300.advent-pro[Git or Done]


---
```{r child="slides/resources.Rmd"}
```
